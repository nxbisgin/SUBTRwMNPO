...
% $Revision: 1.20 $ $Date: 2005/12/28 02:46:46 $
...
fseek(fid,0,-1);
ctf.res4.header = char(fread(fid,8,'uchar'))';
...
%---DATE/TIME
fseek(fid, 778,-1);
ctf.setup.time = char(fread(fid,255,'uchar'))';
fseek(fid,1033,-1);
ctf.setup.date = char(fread(fid,255,'uchar'))';
...
%---RUN NAME
fseek(fid,1360,-1);
ctf.setup.run_name = char(fread(fid,32,'uchar'))';

%---RUN TITLE
fseek(fid,1392,-1);
ctf.setup.run_title = char(fread(fid,256,'uchar'))';

%---SUBJECT
fseek(fid,1712,-1);
ctf.setup.subject = char(fread(fid,32,'uchar'))';

%---OPERATOR
fseek(fid,1744,-1);
ctf.setup.operator = char(fread(fid,32,'uchar'))';

%---SENSOR FILE NAME
fseek(fid,1776,-1);
ctf.setup.sensor_file_name = char(fread(fid,60,'uchar'))';

%---RUN DESCRIPTION & FILTERS
fseek(fid,1836,-1);
run_size = fread(fid,1,'int32');
fseek(fid,1844,-1);
ctf.setup.run_description = char(fread(fid,run_size,'uchar'));
