function [ctf] = ctf_read_markerfile(folder,ctf)

% ctf_read_markerfile - load data from MarkerFile.mrk in .ds folder
%
% [ctf] = ctf_read_markerfile([folder],[ctf]);
%
...
%
% Returns the 'ctf.markers' struct:
%
...
% $Revision: 1.10 $ $Date: 2006/02/01 18:39:57 $
...
number_samples_index = strmatch('NUMBER OF SAMPLES:',markerFileText,'exact');
number_samples = str2num(char(markerFileText(number_samples_index + 1)));
...
HEADER_LINES = 2;
trial = strmatch('LIST OF SAMPLES:',markerFileText,'exact') + HEADER_LINES;
...
for i = find(number_samples)'
    ...
    sample_text_rows = markerFileText( trial(i):[trial(i) + number_samples(i)] );
...
% allocate marker information into the ctf stucture
ctf.markers = struct(...
